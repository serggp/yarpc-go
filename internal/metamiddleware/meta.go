...
// New returns a new YARPC meta middleware.
func New() *Middleware {
	return &Middleware{}
}
...
func update(req *transport.Request, out transport.Outbound) {
	...
	// Reset the transport field to the current outbound transport. Request
	// forwarding in the transport layer (ie yarpcproxy) needs this when copying
